...
func checkGitStatus() error {
	...
	if !strings.Contains(strings.TrimSpace(stdout.String()), "arsiba/tofulint-ruleset-aws") {
		return fmt.Errorf("remote.origin is not arsiba/tofulint-ruleset-aws, got %s", strings.TrimSpace(stdout.String()))
	}
	return nil
}
